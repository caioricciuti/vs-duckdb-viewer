  ...
  implements vscode.CustomReadonlyEditorProvider<DuckDBDocument>
{
  ...
  openCustomDocument(
    ...
    console.log("[DuckDB Viewer] openCustomDocument:", uri.fsPath);
  ...
  async resolveCustomEditor(
    ...
    console.log("[DuckDB Viewer] resolveCustomEditor:", document.uri.fsPath);
    const filePath = document.uri.fsPath;
    const provider = new DuckDBProvider(filePath);
    ...
    webviewPanel.webview.onDidReceiveMessage(async (msg: ExtensionMessage) => {
      try {
        switch (msg.type) {
          case "getSchema":
          case "refresh": {
            if (msg.type === "refresh") await provider.reconnect();
          ...
          case "selectTable": {
            const result = await provider.getTableData(
              msg.table!,
              msg.page ?? 0,
              msg.sortColumn,
          ...
          case "runQuery": {
            ...
            const result = await provider.runQuery(msg.sql!);
            await webviewPanel.webview.postMessage({
              type: "queryResult",
              ...result,
            });
          ...
          case "exportToFile": {
            ...
            const saveUri = await vscode.window.showSaveDialog({
              ...
              defaultUri: vscode.Uri.file(
                filePath.replace(/\.[^.]+$/, `_export.${format}`)
              ),
            });
            if (!saveUri) break;
          ...
          case "requestAttachFile": {
            ...
            await provider.attachFile(attachPath, alias);
            const updatedSchema = await provider.getSchema();
            await webviewPanel.webview.postMessage({
              type: "schemaLoaded",
    ...
    try {
      ...
      const schema = await provider.getSchema();
      await webviewPanel.webview.postMessage({ type: "schemaLoaded", schema });
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err);
      await webviewPanel.webview.postMessage({ type: "error", message });
    }
  ...
  private getHtml(wv: vscode.Webview, filePath: string): string {
    const extensionUri = this.context.extensionUri;
    const styleUri = wv.asWebviewUri(
      vscode.Uri.joinPath(extensionUri, "src", "webview", "style.css")
    );
  ...
  <div class="app">
    <aside class="sidebar">
      ...
      <div class="sidebar-footer">
        <span class="db-name" title="${esc(filePath)}">${esc(fileName)}</span>
      </div>
    </aside>
    ...
    <main class="main">
      <div class="query-bar">
        ...
        <div class="query-actions">
          <button class="btn btn-primary" id="run-btn" title="${modKey}+Enter">
